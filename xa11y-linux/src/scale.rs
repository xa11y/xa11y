...
//!   On X11 this equals [`coordinate_scale`]. On a Wayland session it
...
//!   Session classification mirrors the capture backend selection. A mixed
//!   Wayland/XWayland session uses the Wayland source unless
//!   `XA11Y_LINUX_SCREENSHOT_BACKEND=x11` explicitly selects X11.
//!
...
/// [`coordinate_scale`] on X11; on a Wayland session it comes from a
/// `wl_output` query. Detected once, then cached.
///
/// Selection uses the same override and mixed-session rules as
/// `screenshot::LinuxScreenshot::new`, so the reported scale always belongs
/// to the backend that produced the pixels.
pub fn screenshot_scale() -> f64 {
    static SCALE: OnceLock<f64> = OnceLock::new();
    *SCALE.get_or_init(|| {
        match select_backend("XA11Y_LINUX_SCREENSHOT_BACKEND", "screenshot capture") {
            Ok(DesktopBackend::X11) => coordinate_scale(),
            Ok(DesktopBackend::Wayland) => wayland_output_scale().unwrap_or(1.0),
            Err(_) => 1.0,
        }
    })
}
