...
This page explains the model. The code here illustrates the concepts rather
than serving as a recipe: the [tutorial](/tutorials/first-script/) is the
guided path, the [guides](/guides/install/) are the recipes, and the
[API reference](https://docs.rs/xa11y/) carries the signatures.
...
data is published by the application itself, so it is semantic rather than
pixel-derived: a button is labelled a button because the application says so.
Equally, an application that publishes nothing is invisible to xa11y, and no
amount of cleverness in the library changes that.
...
Element properties: `role`, `name`, `value`, `description`, `bounds`, `states`,
`actions`, `numeric_value`, `min_value`, `max_value`, `stable_id`, `pid`.
Element navigation and snapshots: `children()`, `parent()`, `tree(max_depth)`,
`dump(max_depth)`. Most of these properties are also usable as selector
attributes, which [Selectors](/reference/selectors/) covers.
...
The vocabulary available on a Locator:

**Actions:** `press()`, `focus()`, `blur()`, `toggle()`, `select()`,
`expand()`, `collapse()`, `set_value()`, `set_numeric_value()`, `type_text()`,
`select_text()`, `increment()`, `decrement()`, `show_menu()`,
`scroll_into_view()`.

**Waits:** `wait_visible()`, `wait_hidden()`, `wait_attached()`,
`wait_detached()`, `wait_enabled()`, `wait_disabled()`, `wait_focused()`,
`wait_unfocused()`, `wait_for_state()`, `wait_until()`.

**Queries:** `element()` (single match), `elements()` (all matches),
`exists()`, `count()`.

**Chaining:** `nth(n)`, `first()`, `child(selector)`, `descendant(selector)`.

To inspect element properties or traverse the subtree, call `.element()` first,
because `tree()` and `dump()` live on `Element` rather than `Locator`.

A selector specific enough to survive the interface growing another button is
worth the extra characters. [Selectors](/reference/selectors/#writing-selectors-that-keep-working)
