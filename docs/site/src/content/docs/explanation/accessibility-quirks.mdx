...
Because UIA has no `Application` accessible, xa11y groups a process's
top-level HWNDs into **one synthesized `Application` node** whose children are
those windows — so the window role is still what you see in `App.windows()`,
but it is never the app entry itself (see [Application nodes
(Windows)](/reference/platform-details/#application-nodes-windows)).
...
the MSAA role when it has a clean equivalent. This applies *only* to `Custom`:
an unrecognized UIA control type stays `unknown`, so real gaps in the role map
remain visible.

### Windows: the same grid cell is `DataItem` or `Custom` depending on the framework
...
the presence of `GridItem` identifies a cell on its own. xa11y treats the
`TableItem` pattern as the cell signal for both `DataItem` and `Custom`
elements, and falls back to a structural check (a `DataItem` whose parent is a
`DataItem`) for bridges that expose no table patterns at all, such as
AccessKit's.
...
minimum and maximum are not exposed at all. So a control that looks like a
slider in the tree can still have no numeric value and no range, and stepping
it through the accessibility API is not possible.

WPF is the counter-example on the same platform: `SliderAutomationPeer` and
`ProgressBarAutomationPeer` both implement `IRangeValueProvider`, so identical
control types carry a value and a range there. "Is this a slider?" and "can I
read its value?" are separate questions on Windows.
...
### Windows: Qt dialogs are top-level windows of the process

A `QDialog` opened inside a host process (e.g. a Qt submitter dialog inside
a C++ DCC application) registers with UIA as its **own top-level window**,
sharing the host's PID but living outside the host app's element tree. UIA
has no `Application` accessible, so xa11y groups a process's top-level
windows into one synthesized Application node — the dialog is therefore
reachable through the host app's `App.windows()`, not through
`App.find(...)` by the dialog's name. Do not rely on an app-name lookup
for such a dialog anyway: the synthesized node is named by the process
executable stem when the process can be opened, but falls back to the
representative window's title when it cannot (a restricted or
inaccessible process); either way the dialog name is not the discovery
key. Discovery goes through the process's window list:

```python
app = xa11y.App.by_pid(proc.pid)
dialog = next(
    w for w in app.windows() if w.name.startswith("My Submitter")
)
```

On macOS AX the same dialog is an ordinary child window of the host app, so
a process's `App.windows()` finds it there too.

Both of the following are confirmed against live trees:

- **The dialog's accessible name is the `QApplication` display name, not
  `windowTitle()`.** The dialog window surfaces
  `QGuiApplication::applicationDisplayName()` (typically the app name +
  version), so match on a name *prefix* rather than the window title you
  see in the title bar.
- **Child popups may be hosted elsewhere.** A `QMessageBox` shown from such
  a dialog can appear in the *host* application's tree, not the dialog's
  own subtree. When the owner is unpredictable, search from the system root
  (module-level `xa11y.locator(...)`) and wait for the popup's body text.

### Linux: more than one app node per PID

Linux can surface **more than one application node for one host PID**: the
AT-SPI registry may register an extra top-level window as a separate
accessibility application of its own. `App.list()` can then list two entries
for one process, and a pid match alone cannot tell which one is the
foreground — the entry-level `active` flag disambiguates (`App.find` with a
`focused` predicate sees the same flag). Match a specific node by name when
the platform hands back several:
...
Both claims are exercised by the Rust integration suite: with a second window
open, `foreground_scenario_two_windows` (`xa11y/tests/integ/multi_window.rs`)
asserts that every pid-matching `App.list()` entry is an `Application` node
and that `active` disambiguates — the pid-matching entries report
`is_foreground()` on the one the platform deems frontmost — and
`find_by_focused_resolves_foreground_app` (`xa11y/tests/integ/tree.rs`)
covers the `App.find` with a `focused` predicate. Neither test asserts the
entry count: the multi-registration shape is toolkit-dependent, so the count
is deliberately not part of the contract (see
`by_pid_windows_are_process_complete_on_linux` for the dialog-as-own-node
variant).
...
### egui text fields accept `SetValue` and ignore it

egui's `TextEdit` declares an AccessKit role (`TextInput`,
`MultilineTextInput`, or `PasswordInput`) but never drains the
`Action::SetValue` requests that role invites. Its `DragValue` and `Slider`
widgets do drain them, so the gap is per-widget rather than per-toolkit.

What a caller sees depends on the platform, and none of the three is
reporting incorrectly:

- **macOS** returns success. `accesskit_macos` implements
  `setAccessibilityValue:` as an ObjC `void` method that forwards to
  `do_action`, and neither can signal failure back to the AX client.
- **Windows** returns success. `accesskit_windows` implements
  `IValueProvider::SetValue` on top of an `ActionHandler::do_action` that
  returns `()`, so UIA reports `S_OK` once the request is *dispatched*, not
  once the app applies it.
- **Linux** raises. The AT-SPI bridge in `accesskit_unix` never exposes the
  `EditableText` interface, only `Value.SetCurrentValue`, so the
  `EditableText.SetTextContents` call xa11y makes is rejected with
  `UnknownInterface` and surfaces as `TextValueNotSupported`.

This is the general shape of the AccessKit action pipeline, not something
specific to text: an action request is fire-and-forget on the two platforms
whose native API has no failure channel. A success return from `set_value()`
on an AccessKit-backed app means the request reached the app, so read the
value back if you need to know it landed.
...
- **macOS reports points, not pixels.** On a Retina display 1 point = 2 physical
  ...
  scale factor.
- **Windows and Linux report logical (device-independent) coordinates.** Use a
  screenshot's explicit coordinate conversion methods to bridge them to the
  actual returned image; `Screenshot.scale` is only a single-display
  compatibility hint. On Windows a mixed-DPI
  multi-monitor desktop preserves each monitor's physical origin in the
  logical space (only its extent is divided by that monitor's scale), so the
  monitors' logical rects are non-overlapping; a scaled monitor placed left of
  or above the primary preserves its origin but not the full span toward the
  primary, so its rect stops short and can leave a logical gap (gap points
  resolve via the physical query). On Linux the
  scale is exact where reliably detectable and falls back to `1.0`
  (coordinates then physical) otherwise. See
  [platform details](/reference/platform-details/#coordinates-and-hidpi).
...
- xa11y's own [Platform Details](/reference/platform-details/) and
  [Testing in CI](/guides/ci/) guides.
