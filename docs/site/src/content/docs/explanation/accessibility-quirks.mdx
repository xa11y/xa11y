...
- **macOS reports points, not pixels.** On a Retina display 1 point = 2 physical
  ...
  scale factor.
- **Windows reports physical desktop pixels.** Bounds, input, window geometry,
  and capture regions share one continuous pixel space across displays.
  `Screenshot.scale` is `1.0`.
- **Linux reports logical coordinates where it can detect the scale.** Use a
  screenshot's explicit coordinate methods to map them to image pixels.
  `Screenshot.scale` is a compatibility hint; Linux uses `1.0` when it cannot
  detect a reliable scale.
