...
accessibility application of its own. Since 0.14, `App` consistently means
the whole process: `App.list()` returns one entry per resolved PID, and
`windows()`, `children()`, `tree()`, locators, and subscriptions include all
of that process's native registrations. Code that previously selected one
same-PID registration by its name should instead select the process and use a
window or descendant selector.
...
asserts that the pid-matching `App.list()` entry is an `Application` node,
that the complete window set remains reachable, and that `active` identifies
the platform's frontmost window. The
...
- **Windows and Linux report logical (device-independent) coordinates**, with
  `Screenshot.scale` bridging to physical pixels. On Windows a mixed-DPI
