...
### 2. Semantic platform APIs

Element actions use the platform accessibility API by default. Window actions may use a native window-manager protocol when accessibility has no equivalent. That backend must report its capability and prove the accessibility element's native-window identity before it mutates anything. If no semantic API is available, leave the action out of the element's `actions` list. Input simulation lives in its own surface (`InputSim`) and is never reached from an element or window action.
