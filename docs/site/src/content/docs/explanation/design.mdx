...
The one legitimate reason to hold the lock is calling back into the host language, for example a Python predicate in `wait_until` or `App.find`. Reacquire it per callback rather than for the whole loop. A missing `allow_threads` on a blocking path is a correctness bug rather than a missed optimisation.
...
The structured carrier is the `Diagnosis` struct in `xa11y-core/src/error.rs`, whose module docs describe the pattern: enrich at the terminal failure site, bound every payload, and never let diagnosis collection mask the original error. What this produces for a consumer is described in [Errors & Diagnosis](/explanation/errors-and-diagnosis/).
