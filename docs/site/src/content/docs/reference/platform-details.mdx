...
| xa11y Action | macOS | Linux (AT-SPI) | Windows (UIA) |
| --- | --- | --- | --- |
| `Activate` | activate app *(set AXFrontmost)*, restore if minimized, then AXRaise | Component.GrabFocus *(same path as Focus)* | restore if minimized, then SetForegroundWindow and SetFocus |
| `Minimize` | clear AXFullScreen when fullscreen, then set AXMinimized=true | *(unsupported)* | WindowPattern.SetWindowVisualState(Minimized) |
| `Maximize` | *(unsupported)* | *(unsupported)* | WindowPattern.SetWindowVisualState(Maximized) |
| `EnterFullscreen` | set AXFullScreen=true | *(unsupported)* | *(unsupported)* |
| `Restore` | clear AXMinimized; set AXFullScreen=false | *(unsupported)* | WindowPattern.SetWindowVisualState(Normal) |
