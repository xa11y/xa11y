...
A missing platform permission is not a launch failure. When the accessibility
provider cannot be constructed, `start()` propagates the underlying
`xa11y.PermissionDeniedError` with its grant instructions, after stopping the
process it spawned.
