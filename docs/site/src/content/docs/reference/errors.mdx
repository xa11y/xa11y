...
| --- | --- |
| [Input simulation](/guides/input/) | `Unsupported`, `NoElementBounds`, `InvalidActionData`, `Platform`; on Linux, `PermissionDenied` when the `input` group is missing. macOS cannot report a missing Input Monitoring grant, so it drops the events silently |
