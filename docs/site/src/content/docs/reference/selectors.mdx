...
way, because a role token accepts letters, digits, and underscores alone.
...
The case-insensitive operators compare after an ASCII-oriented lowercase
mapping, so for names that differ only in a locale-specific case edge case, use
the case-sensitive `=` operator with the precise string.
...
| `value` | String |
| `description` | String |
| `bounds` | JSON object, `{"x":…,"y":…,"width":…,"height":…}` |
...
Any other attribute name falls through to the element's platform-specific `raw`
...
Make selectors specific enough that they stay stable as the interface evolves.
`button` alone can start matching the wrong element or returning several
matches once the application grows another button, so scope it: prefer
`button[name='Cancel']` inside a named group or window.
