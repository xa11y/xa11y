...
Window states are also `Option<bool>`: `None` means the platform cannot
report that state. Sway cannot report `minimized` or a distinct `maximized`
state, other Wayland compositors expose only AT-SPI's minimized bit, macOS
cannot report maximized, and Windows cannot report fullscreen. An element whose state is unknown
matches nothing, so `[minimized='false']` and `[minimized='true']` both
exclude windows the platform cannot describe:

| Attribute | Notes |
| --- | --- |
| `minimized` | Absent when unknown / not a window |
| `maximized` | Absent when unknown / not a window |
| `fullscreen` | Absent when unknown / not a window |
