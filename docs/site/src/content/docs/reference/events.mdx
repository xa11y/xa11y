...
| `ValueChanged` | `value_changed` | `valueChanged` | An element's value changed: slider position, text contents, checkbox state, spin button, progress. |
...
Variants carry a payload only where the data is present on every supporting
platform. For everything else, re-query the `target` element after receipt.
...
| `WindowActivated` | Windows has no first-class UIA event. It is inferred from focus changes. |
| `StateChanged` | Linux delivers all state bits. Windows delivers `IsEnabled`, `ToggleState`, `ExpandCollapseState`, and `SelectionItem_IsSelected`. macOS delivers `Checked` and `Busy` alone; `Enabled` is not deliverable through any public app-level macOS notification and will never fire there. |
...
| `timestamp` | monotonic instant | Time of receipt |
...
`Subscription.wait_for` raises `Timeout` when no matching event arrives. The
diagnosis reports how many non-matching events were seen and distinguishes a
quiet stream from a disconnected one. See
