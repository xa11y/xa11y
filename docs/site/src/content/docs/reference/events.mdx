...
| `StateChanged` | Linux delivers all state bits. Windows delivers `IsEnabled`, `ToggleState`, `ExpandCollapseState`, and `SelectionItem_IsSelected`. macOS delivers `Checked` and `Busy` alone. No public app-level macOS notification reports `Enabled`, so it will never fire there. |
