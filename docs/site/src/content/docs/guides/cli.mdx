...
Scrolling actions take an optional numeric value (default 1.0):

```bash
xa11y action scroll-down "list[name='Messages']" --app Slack
xa11y action scroll-down "list[name='Messages']" --app Slack --value 3.0
```
...
| `scroll-down` | Scroll down (optional `--value` for amount) |
| `scroll-right` | Scroll right (optional `--value` for amount) |
