...
GUIs, two distinct web renderers, and — with Windows Forms — a first-party
Microsoft UIA provider rather than a third-party bridge). The toolkit-specific
quirks this surfaces are documented in
...
`macos × {cocoa, tauri, qt, egui}`, `windows × {qt, egui, winforms}`), each standing up
