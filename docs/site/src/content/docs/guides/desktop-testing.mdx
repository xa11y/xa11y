...
- **The UI may register as more than one accessibility application.** Linux can surface *more than one* application node for one host PID — a toolkit may register an extra top-level window as a separate accessibility application under its own app entry (see [Accessibility API Quirks](/explanation/accessibility-quirks/)). On Windows and macOS the process maps to exactly one Application node, so a PID there is unambiguous; on Linux match a specific node with a predicate instead of polling `App.list()` by hand:
