...
<Tabs syncKey="lang">
  <TabItem label="Rust">
    ...
    fn calculator_multiplication() -> Result<()> {
        ...
        result.wait_until(
            |node| node.and_then(|n| n.value.as_deref()) == Some("56"),
            timeout,
        )?;
        assert_eq!(result.node()?.value.as_deref(), Some("56"));
  ...
  <TabItem label="Python">
    ...
    def test_calculator_multiplication(calculator):
        ...
        result.wait_until(lambda node: node and node.value == "56", timeout=5.0)
        assert result.node().value == "56"
    ```
  </TabItem>
</Tabs>
