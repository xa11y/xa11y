...
After changing permissions, restart your terminal for them to take effect. `App::by_name()` (Rust) or `xa11y.locator()` (Python) will return a `PermissionDenied` error with setup instructions if either permission is missing.
...
<Tabs syncKey="lang">
  ...
  <TabItem label="Python">
    ...
    # Connect to the Calculator app via locator
    # (raises PermissionDeniedError if accessibility is not enabled)
    calc = xa11y.locator('application[name="Calculator"]')

    # Navigate the tree
    app_element = calc.element()
    for window in app_element.children():
        print(f"{window.role}: {window.name}")

    # Find all buttons via locator
    buttons = calc.descendant("button").elements()
    print(f"Found {len(buttons)} buttons")

    # Read properties from a specific element
    btn = calc.descendant("button[name='=']").element()
    ...
    # Type into a text field
    calc.descendant("text_field[name='Input']").type_text("42")
    ...
    # List all running apps
    for app in xa11y.locator("application").elements():
