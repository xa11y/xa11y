...
**The capture owns annotation coordinates.** Full-desktop and region captures
carry one mapping segment per captured display, so the public `annotate`
primitive and the locator-based annotated screenshot path place and omit the
same rectangles. A box crossing displays is split into image rectangles and
gets one badge. `scale` remains for compatibility; it is not used as a
whole-desktop transform.

Wayland is the remaining platform limitation: the screenshot portal returns a
composited image without a standardized per-output transform. xa11y exposes
the portal image's best-known 1:1 mapping, which can be approximate under
