...
`stable_id` is stable across queries but **not guaranteed unique** — on macOS some toolkits (notably Qt) stamp one `AXIdentifier` across a whole subtree, and on Windows `AutomationId` is only unique among siblings. Don't use it as a map key, and expect `[stable_id="…"]` selectors to match several elements. See [`stable_id` is stable, not unique](/guides/platform-details/#stable_id-is-stable-not-unique).
