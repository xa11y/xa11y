...
<img src="/concepts.svg" alt="Diagram showing xa11y's three concepts: Read (App → .elements() → Element → .children()/.parent()), Act (App → .locator() → Locator → Actions/Waits/.element()/.elements()), and Listen (App → subscribe() → Event Stream)" style="width:100%;max-width:720px;margin:1rem auto;display:block" />

xa11y is built around three core types:

- **App** — entry point for interacting with an application. Construct via `App::from_name()`, `App::from_pid()`, or `App::all()`. An App provides two paths: `app.elements()` for read-only snapshots, and `app.locator(selector)` for actions and waits.
...
`app.elements()` captures a frozen copy of the accessibility tree and returns the root `Element`. All reads come from this snapshot with zero platform calls. Call `elements()` again for a fresh one.

An `Element` is a cheap, cloneable handle into the snapshot. Navigate with `children()` and `parent()`.
...
`elements()` captures the tree at a single point in time. The snapshot never updates implicitly.

```rust
let root = calc.elements()?;       // snapshot captured here
println!("{:?}", root.children()[0].name);  // reads from snapshot

// UI changed? Get a fresh snapshot:
let root = calc.elements()?;       // new snapshot
```
...
// Read properties via .element() snapshot
...
Unlike Element, which reads from a frozen snapshot, Locator always works with the latest UI state.
