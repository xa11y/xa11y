...
<img src="/concepts.svg" alt="Diagram showing xa11y's three concepts: Read (App → .elements() → Element → .children()/.parent()), Act (App → .locator() → Locator → Actions/Waits/.element()/.elements()), and Listen (App → .subscribe() → Subscription → recv()/wait_for()/iter())" style="width:100%;max-width:720px;margin:1rem auto;display:block" />
...
let safari = App::by_name(provider()?, "Safari")?;

// Connect by PID
let app = App::by_pid(provider()?, 1234)?;

// List all running apps
let all = App::list(provider()?)?;
```
