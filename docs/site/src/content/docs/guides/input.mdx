...
| Linux (Wayland) | `xdg-desktop-portal` with the **RemoteDesktop** interface (libei). User consent prompt on first use; persisted thereafter when the portal supports `persist_mode=PERMANENT`. |
...
xa11y picks the Linux backend at runtime: if `DISPLAY` is set we drive XTest; otherwise `WAYLAND_DISPLAY` triggers the portal/libei path. There is no compile-time feature flag — both backends are always built into `xa11y-linux`.
