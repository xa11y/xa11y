...
| `qt` | PySide6 (Qt) | ✓ | ✓ | ✓ | compat, actions, events, errors |
...
Which suite drives which app varies. The AccessKit row is covered primarily by
...
suite also targets it for compat and actions. GTK event subscription is
exercised through that same Rust suite over AT-SPI2 rather than duplicated in
the GTK Python suite. Electron is driven by the JS suite, which additionally
covers `AccessibilityNotEnabled` detection by launching the app *without*
