...
[strands-xa11y](https://github.com/xa11y/strands-xa11y) (`pip install strands-xa11y`) packages that same loop for a [Strands](https://strandsagents.com) agent rather than for a runtime that shells out, as a single tool named `use_desktop`. The model reads a snapshot of the tree with a ref on every line, then acts by ref or by [selector](/reference/selectors/). Its `inspect_desktop` variant is the same tool with the acting half removed from the schema, for an agent that should observe and never touch. That repository documents the tool surface and the consent prompts around acting on a real desktop; the [platform permissions](/guides/install/) underneath are xa11y's.
...
- **[strands-xa11y](https://github.com/xa11y/strands-xa11y)** gives a [Strands](https://strandsagents.com) agent xa11y as a desktop-control tool, with a read-only variant for agents that observe.
