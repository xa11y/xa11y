...
[Strands](https://strandsagents.com) is AWS's open-source SDK for building agents in Python or TypeScript, where a capability reaches the model as a tool: a function whose schema the model is shown and decides when to call. [strands-xa11y](https://github.com/xa11y/strands-xa11y) (`pip install strands-xa11y`) is the desktop in that shape. It packages the same loop as one tool named `use_desktop`, so an agent gains desktop control by listing it in `tools=[...]` instead of shelling out to a CLI. The model reads a snapshot of the tree with a ref on every line, then acts by ref or by [selector](/reference/selectors/). Its `inspect_desktop` variant is the same tool with the acting half removed from the schema, for an agent that should observe and never touch. That repository documents the tool surface and the consent prompts around acting on a real desktop; the [platform permissions](/guides/install/) underneath are xa11y's.
